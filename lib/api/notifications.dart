...
class PushNotifications {
  ...
  void handleInAppMessage(RemoteMessage? message) async {
      ...
      if (message.data["type"] == "widget-botw") {
        ...
        if (index == -1) {
          ...
          WidgetKit.setItem('snippetsResponsesData',
              jsonEncode(oldResponsesMap), 'group.kazoom_snippets');
        ...
        Map<String, dynamic>? oldAnswer = oldAnswers.firstWhere(
            (element) => element["uid"] == message.data["uid"],
            orElse: () => {});
        ...
        String responseString =
            "${message.data["displayName"]}|${message.data["question"]}|${message.data["response"]}|${message.data["snippetId"]}|${message.data["userId"]}|${message.data["snippetType"] == "anonymous"}|${message.data["answered"]}";
        if (oldResponses.contains(responseString)) {
          return;
        }
        for (var response in oldResponses) {
          if (response.split("|")[3] == message.data["snippetId"] &&
              response.split("|")[6] == "false") {
            oldResponses[oldResponses.indexOf(response)] =
                "${response.split("|")[0]}|${response.split("|")[1]}|${response.split("|")[2]}|${response.split("|")[3]}|${response.split("|")[4]}|${response.split("|")[5]}|true";
          }
        }
    ...
    if (message.data['type'] == "snippetAnswered" &&
        currentPage == "responses") {
      return;
    }

    if (message.data['type'] == "discussion" &&
        currentPage.split("-")[0] == "discussion" &&
        currentPage.split("-")[1] == message.data['snippetId'] &&
  ...
  void handleMessage(RemoteMessage? message) async {
    ...
    if (!isSignedIn) {
      ...
      if (message.data['type'] == "question") {
        ...
        bool hasResponded =
            await FBDatabase(uid: FirebaseAuth.instance.currentUser!.uid)
                .didUserAnswerSnippet(message.data['snippetId']);
        if (hasResponded) {
          // navigatorKey.currentState?.push(
          //   CustomPageRoute(
          //     builder: (BuildContext context) {
      ...
      } else if (message.data['type'] == "reminderToVoteBOTW" ||
          message.data['type'] == "botw voting") {
        // navigatorKey.currentState?.pushAndRemoveUntil(
        //   CustomPageRoute(
        //     builder: (BuildContext context) {
